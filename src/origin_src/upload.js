...
export function mediaUploadComplete(error, uploadData) {
  const mediaObject = uploadData.mediaObject,
        uploadedFile = uploadData.uploadedFile;
  return dispatch => {
    if (error) {
...
export function uploadToMediaFolder(tabContext, fileMetaProps) {
  return (dispatch, getState) => {
    ...
    return source.fetchMediaFolder({
      ...
      dispatch(uploadPreflight(tabContext, _objectSpread({}, fileMetaProps, {
        bookmark
      })));
    }).catch(e => {
...
function readUploadedFileAsDataURL(file) {
  let reader = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : new FileReader();
