...
$.fn.formSubmit = function (options) {
  ...
  this.submit(function (event) {
    ...
    if (onSubmit) {
      ...
      $.each(["success", "error"], function (i, successOrError) {
        ...
        options[successOrError] = function () {
          loadingPromise[
            successOrError === "success" ? "resolve" : "reject"
          ].apply(loadingPromise, arguments);
    ...
    const xhrError = function (data, request) {
      ...
      if (
        $formObj.parents("html").get(0) == $("html").get(0) &&
        options.formErrors !== false
      ) {
    ...
    if (options.noSubmit) {
      ...
      $.ajaxJSONPreparedFiles.call(this, {
        ...
        files: $.isFunction(options.files)
          ? options.files.call($form)
          : options.files,
      ...
      doUploadFile &&
      $.handlesHTML5Files &&
      $form.hasClass("handlingHTML5Files")
    ) {
    ...
    this.find(":input").each(function () {
      ...
      if (name in data) {
        if (name) {
          if (
            inputType == "hidden" &&
            $obj.next("input:checkbox").attr("name") == name
          ) {
            // do nothing
...
$._stripObjectName = function (data, object_name, include_original) {
  ...
  for (const i in data) {
    ...
    if ((found = original_name.indexOf(object_name + "[") === 0)) {
      ...
      short_name =
        short_name.substring(0, closing) + short_name.substring(closing + 1);
      if (data instanceof Array) {
...
$.fn.validateForm = function (options) {
  ...
  if (options.object_name) {
    ...
    options.date_fields = $._addObjectName(
      options.date_fields,
      options.object_name
    );
  ...
  if (options.required) {
    ...
    $.each(required, (i, name) => {
      if (!data[name]) {
        ...
        errors[name].push(
          I18n.t("errors.required", "Required field") +
  ...
  if (options.date_fields) {
    $.each(options.date_fields, (i, name) => {
      ...
      if ($item.length && $item.data("invalid")) {
        ...
        errors[name].push(
          I18n.t("errors.invalid_datetime", "Invalid date/time value")
        );
      }
    });
  }
  if (options.numbers) {
    $.each(options.numbers, (i, name) => {
      ...
      if (isNaN(val)) {
        ...
        errors[name].push(
          I18n.t("errors.invalid_number", "This should be a number.")
        );
      }
    });
  }
...
$.fn.formErrors = function (data_errors, options) {
  ...
  $.each(data_errors, (i, val) => {
    ...
    if (
      $form.find(":input[name='" + i + "'],:input[name*='[" + i + "]']")
        .length > 0
    ) {
      $.each(val, (idx, msg) => {
        if (!errors[i]) {
  ...
  if (hasErrors) {
    if (options && options.onFormError)
      options.onFormError.call($form, errorDetails);
...
$.fn.errorBox = function (message, scroll, override_position) {
  if (this.length) {
    ...
    const cleanup = function () {
      ...
      const srError = _.find(
        $screenReaderErrors,
        (node) => $(node).text() == $box.text()
      );
      $box.remove();
      if (srError) {
        $(srError).remove();
      ...
      if (this.id != "pseudonym_session_unique_id_forgot") {
        label.append(
          $('<span aria-hidden="true" />')
            ...
            .attr(
              "title",
              I18n.t("errors.field_is_required", "This field is required")
            )
        );
      }
    });
