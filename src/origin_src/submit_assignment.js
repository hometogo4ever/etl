...
var SubmitAssignment = {
  ...
  moreToolsListClickHandler(event) {
    ...
    SubmitAssignment.homeworkSubmissionLtiContainer.embedLtiLaunch(
      tool.get("id")
    );

    const width =
      tool.get("homework_submission").selection_width ||
      tool.get("selection_width");
    const height =
      tool.get("homework_submission").selection_height ||
      tool.get("selection_height");
...
$(document).ready(function () {
  ...
  SubmitAssignment.homeworkSubmissionLtiContainer =
  ...
  if (!ENV.use_rce_enhancements) {
    ...
    keyboardShortcutsView
      .render()
      .$el.insertBefore($(".switch_text_entry_submission_views:first"));
  }
  ...
  submissionForm.submit(function (event) {
    const self = this;
    const $turnitin = $(this).find(".turnitin_pledge"); // 이 과제는 내 자신의 독창적인 작업입니다.(checkbox)
  ...
  $(".switch_text_entry_submission_views").click(function (event) {
    ...
    RichContentEditor.callOnRCE(
      $("#submit_online_text_entry_form textarea:first"),
      "toggle"
    );
  ...
  const fileBrowser = (
    <FileBrowser
      selectFile={(fileInfo) => {
        ...
        $("#submission_attachment_ids").data(
          String(fileInfo.id),
          fileInfo.name
        );
  ...
  $(".add_another_file_link")
    .click(function (event) {
      ...
      $("#submission_attachment_blank")
        ...
        .attr(
          "name",
          "attachments[" + ++submissionAttachmentIndex + "][uploaded_data]"
        );
      ...
      (data, textStatus) => {
        ...
        tree.on("activate-file", (file_id) => {
          $("#submit_google_doc_form")
            ...
            .val(file_id);
          const submitButton = $("#submit_google_doc_form").find(
  ...
  $(".submission_attachment input[type=file]").live("change", function () {
    ...
    $.screenReaderFlashMessage(
      I18n.t("File selected for upload: %{filename}", { filename })
    );
...
$(document).ready(() => {
  $("#submit_media_recording_form .submit_button")
    .attr("disabled", true)
    .text(
      I18n.t("messages.record_before_submitting", "Record Before Submitting")
    );
  $(
    "#media_media_recording_submission_holder .record_media_comment_link"
  ).click((event) => {
    event.preventDefault();
    $("#media_media_recording_submission").mediaComment(
      "create",
      "any",
      (id, type) => {
        $("#submit_media_recording_form .submit_button")
...
function uploadFileFromUrl() {
  ...
  const uploadPromise = uploadFile(preflightUrl, preflightData, null)
    ...
    .catch((error) => {
      ...
      $tools
        .find(".submit")
        .text(I18n.t("file_retrieval_error", "Retrieving File Failed"));
      $.flashError(
        I18n.t(
